...
export const ja_JP: TranslationMap = {
  ...
  channels: {
    ...
    pairing: {
      ...
      approve: "承認",
      dismiss: "却下",
      approveAria: "{channel}（アカウント {account}）の {sender} を承認",
      dismissAria: "{channel}（アカウント {account}）の {sender} を却下",
      requested: "要求日時 {ago}",
      expires: "有効期限 {ago}",
      senderDetails: "送信者の詳細",
      missingPermission:
        "この接続には operator.pairing アクセス権がないため、DM要求を確認できません。",
      noAccounts: "DM送信者ペアリングを使用するチャンネルアカウントが設定されていません。",
      noRequests: "保留中のDMアクセス要求はありません。",
      noFilteredRequests: "これらのフィルターに一致する保留中の要求はありません。",
      limits:
        "要求は {minutes} 分後に期限切れになります。各チャンネルアカウントは最大 {count} 件の保留中要求を保持できます。",
      detailTitle: "DMアクセス",
      detailSubtitle:
      ...
      approveExplanation:
        "これにより、送信者がダイレクトメッセージでエージェントと会話できるようになります。グループアクセスは付与されません。",
      dismissExplanation:
        "これは現在のリクエストを削除しますが、送信者をブロックはしません。送信者は後でアクセスを再度リクエストできます。",
      notifyRequester: "承認後にリクエスト元へ通知する",
      makeCommandOwner: "この送信者を最初のコマンドオーナーにも設定する",
      commandOwnerHelp:
        "コマンドオーナーは特権コマンドを実行し、危険なアクションを承認できます。このオプションはオーナーが未設定の場合にのみ利用できます。",
      commandOwnerNeedsAdmin:
        "コマンドオーナーが設定されていません。この接続で最初のオーナーを割り当てるには operator.admin が必要です。",
      approvedNotice: "DMアクセスを承認しました。",
      approvedOwnerNotice: "DMアクセスを承認し、最初のコマンドオーナーを設定しました。",
      approvedNotificationFailedNotice:
        "DMアクセスを承認しましたが、リクエスト元への通知を配信できませんでした。",
      approvedOwnerFailedNotice:
        "DMアクセスを承認しましたが、最初のコマンドオーナーを設定できませんでした。",
      approvedFollowupsFailedNotice:
        "DMアクセスを承認しましたが、リクエスト元への通知とコマンドオーナーの設定の両方が失敗しました。",
      dismissedNotice:
  ...
  sessionsView: {
    ...
    deleteAllArchivedConfirm:
      "{count} 件のアーカイブ済みスレッドとそのトランスクリプトを削除しますか？",
    ...
    noArchivedSessions: "アーカイブ済みのセッションはありません。",
    ...
    cloudWorkerPlacementConflict: "クラウドワーカー: {state} · 1 件のワークスペース競合",
    cloudWorkerPlacementConflicts: "クラウドワーカー: {state} · {count} 件のワークスペース競合",
    cloudWorkerDescendantConflict: "クラウドワーカーの子: 1 件のワークスペース競合",
    cloudWorkerDescendantConflicts: "クラウドワーカーの子: {count} 件のワークスペース競合",
  ...
  configView: {
    ...
    sessionObserver: {
      title: "セッションオブザーバー",
      hint: "長時間実行中のセッションを小さなユーティリティモデルで要約します。",
      toggle: "セッションオブザーバー",
      toggleHint: "購読中の Control UI セッションのライブステータスダイジェストを生成します。",
      resolvedModel: "解決済みの小さいモデル",
      modelAuto: "自動（{model}）",
      modelConfigured: "設定済み（{model}）",
      modelDisabled: "無効",
      modelUnavailable: "利用不可（小さいモデルなし）",
      modelPicker: "小さいモデル",
      modelPickerHint: "オブザーバーダイジェストやその他の短いユーティリティタスクに使用します。",
  ...
  subtitles: {
    ...
    advanced: "残りのすべての設定セクションと生ファイルエディター。",
  ...
  custodian: {
    ...
    sensitivePlaceholder: "機密性の高い値を入力…",
    sensitiveReply: "機密性の高い返信を送信しました",
    send: "送信",
    thinking: "OpenClawが考えています",
    earlier: "以前",
    requestFailed: "OpenClawは返信できませんでした。もう一度お試しください。",
    connectionChanged:
      "Gateway接続が変更されました。このセットアップを続けるには再試行してください。",
    sessionRestarted:
      "{error} OpenClawは新しいセッションを開始しました。以前のメッセージはコンテキストとして残っています。",
    unsupportedGateway: "OpenClawでセットアップを続けるにはGatewayを更新してください。",
    history: {
      button: "履歴",
      title: "最近の変更",
      ...
      opaqueChange: "表示可能な構成パスの変更なしに、フォーマットまたはコメントが変更されました。",
      sources: {
  ...
  mcpServers: {
    ...
    configUnavailable: "構成を利用できません。更新してから再試行してください。",
  ...
  labsPage: {
    intro: "Labs にはリリース間で変更・破損・消失する可能性がある実験的機能が含まれています。",
    sectionTitle: "実験的機能",
    ...
    swarm: {
      title: "Swarm",
      description: "Code Modeにサブエージェントのグループを並列でオーケストレーションさせます。",
  ...
  appsPage: {
    ...
    cards: {
      ...
      appleWatch: {
        title: "Apple Watch",
        desc: "手元でひと目で見られるチャットとクイック返信。",
      },
      wearOs: {
        title: "Wear OS",
        desc: "AndroidコンパニオンがOpenClawをあなたの腕時計へ拡張します。",
      },
      macos: {
        title: "macOS",
        desc: "Gatewayのメニューバーコンパニオン — 通知、承認、クイックチャット。",
      },
      windows: {
        title: "Windows",
        desc: "WindowsコンパニオンがあなたのPCをOpenClawノードとして接続します。",
      },
      linux: {
        title: "Linux",
      ...
      chrome: {
        title: "Chrome拡張機能",
        desc: "OpenClawに既存のChromeを操作させましょう — タブ、ページ、フォーム。",
      },
      plugins: {
        title: "プラグインとClawHub",
        desc: "コミュニティのチャンネル、ツール、スキルでOpenClawを拡張します。",
      },
    },
  },
  presence: {
  ...
  profilePage: {
    ...
    identity: {
      ...
      linkedEmailsDescription: "このプロフィールに接続されたメールアドレスです。",
  ...
  board: {
    ...
    emptyHint:
      "エージェントがここにウィジェットをピン留めできます。ステータスカードを頼んでみましょう。",
    ...
    announcement: {
      ...
      granted: "ウィジェットへのアクセスが許可されました。",
      rejected: "ウィジェットアクセスが拒否されました。",
    },
    widget: {
      cellLabel:
        "ダッシュボードウィジェット: {title}。矢印キーで移動できます。Altキーを押しながら矢印キーを押すと移動します。",
      ...
      appStaleDetail:
        "そのサーバー、リソース、または元となるトランスクリプトが利用できなくなりました。",
      retry: "再試行",
      frameResolverMissing: "ウィジェットのコンテンツは利用できません。",
      sandboxUnavailable: "ウィジェットのサンドボックスホストが利用できません。",
      frameAuthorizationFailed: "更新を繰り返し試みた後、ウィジェットの認証に失敗しました。",
      sandboxOriginRequired:
        "更新を繰り返し試みた後、ウィジェットの認証に失敗しました。gateway がウィジェットのサンドボックスポートをルーティングしないリバースプロキシまたはトンネルの背後で動作している場合は、mcp.apps.sandboxOrigin にサンドボックスリスナーへルーティングされる専用のパブリックオリジンを設定してください。",
  ...
  workboard: {
    ...
    dashboardEmpty:
      "ダッシュボードはまだありません — 稼働中のエージェントがウィジェットをピン留めできます。",
    openSession: "セッションを開く",
  ...
  chat: {
    ...
    workspaceConflict: {
      titleOne: "クラウドワークスペースの競合が1件",
      titleMany: "クラウドワークスペースの競合が{count}件",
      description:
        "OpenClaw はローカル版を保持し、その他のクラウドの変更を適用しました。ステージされた結果を確認するか、競合したパスについてクラウド版を採用してください。",
      morePaths: "他 +{count} 件のパス",
      stagedResult: "ステージされたクラウドの結果",
      copyStagedResult: "ステージされた結果の参照をコピー",
      inspectCloud: "最初のクラウド版を確認",
      takeCloud: "最初のクラウド版を採用",
      copyInspectCommand: "クラウド確認コマンドをコピー",
      copyTakeCommand: "クラウド採用コマンドをコピー",
      commandHelp:
        "これらは Bash または zsh（Windows では Git Bash）で実行してください。inspect でパスが存在しないと表示された場合、クラウドで削除されています。確認のうえ、ローカルのパスを手動で削除してください。checkout がファイル/ディレクトリの競合を報告した場合、妨げているローカルのパスを移動または削除してから再試行してください。ステージされた参照が見つからない場合、この通知は古くなっています。ローカルのパスは変更しないでください。",
      commandsUnavailable:
      ...
      eventTitleOne: "クラウドの結果を適用しました（競合1件）",
      eventTitleMany: "クラウドの結果を適用しました（競合{count}件）",
      eventDescription:
        "これらのパスについてはローカル版が保持され、その他のクラウドの変更が適用されました。",
    },
    compaction: {
      label: "圧縮された履歴",
    ...
    messages: {
      ...
      rewindUnavailable: "エージェントの動作中は巻き戻しを使用できません",
      forkUnavailable: "エージェントの動作中はフォークを使用できません",
      showLess: "表示を減らす",
      showMore: "もっと表示",
      unknownDate: "不明な日付",
      voiceNote: "ボイスメモ",
    ...
    observer: {
      ...
      boardSinceYouLeft: "退出以降",
      health: {
        "on-track": "順調",
        grinding: "作業中",
        stuck: "行き詰まり",
        "waiting-on-user": "あなたを待っています",
        "wrapping-up": "まとめ中",
        done: "完了",
        failed: "失敗",
      },
    ...
    composer: {
      ...
      offlineHint: "オフライン — メッセージはキューに追加され、接続が回復すると送信されます。",
      ...
      cameraInput: "カメラ",
      cameraListUnsupported: "このブラウザはカメラを一覧表示できません。",
      cameraNoneFound: "カメラが見つかりませんでした。",
      cameraPageInactive: "このページが非アクティブの間、カメラは利用できません。",
      ...
      dictationFinalizationTimedOut:
        "最後の途中経過が確定される前にディクテーションが停止しました。",
    ...
    sessionDiscussion: {
      ...
      requiresWriteAccess:
        "このディスカッションを開くにはオペレーターの書き込みアクセスが必要です。",
  ...
  cron: {
    ...
    form: {
      ...
      script: "スクリプト",
      readOnlyPayloadHelp:
        "このペイロードはControl UIの外部で作成されました。その内容は読み取り専用のまま維持され、他の変更を保存しても保持されます。",
